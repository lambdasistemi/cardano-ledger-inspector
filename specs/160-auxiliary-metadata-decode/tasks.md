...
- [X] T160 Add a hermetic RED regression for
...
- [X] T161 Enrich successful `tx.intent` responses in the Haskell/WASI wrapper
      with the lossless tagged metadata tree while preserving other operations
      and error behavior.
- [X] T162 Document the recursive metadata union in the result schema, API
      contract, example, and generated OpenAPI artifact where applicable.
- [X] T163 Prove GREEN with `just check-intent`, `just check-openapi`,
