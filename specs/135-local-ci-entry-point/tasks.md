...
- [X] T001 Add the `just ci` and `check-ci-drift` recipes in `justfile`, covering every executable flake target required by `.github/workflows/ci.yml`.
- [X] T002 Add `scripts/check-ci-entry-point.sh` to compare the workflow and local-CI target sets, with a non-zero drift failure and a controlled negative proof.
