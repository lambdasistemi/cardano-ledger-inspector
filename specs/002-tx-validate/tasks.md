...
- [ ] T012 Run `just check-openapi` and `just check-swagger` to verify `specs/001-ledger-functional-layer/openapi/cardano-ledger-functional.openapi.json`
...
- [ ] T013 [P] [US1] Add a `tx.validate` complete-context smoke request fixture in `specs/001-ledger-functional-layer/fixtures/tx-validate-complete-request.json`
- [ ] T014 [US1] Add jq assertions for valid/invalid validation result shape in `flake.nix`
- [ ] T015 [US1] Add determinism assertions for repeated `tx.validate` runs in `flake.nix`

### Implementation for User Story 1

- [ ] T016 [US1] Implement `validateTxJson` result rendering in `nix/wasm/tx-inspector/wasm-tx-inspector/src/Conway/Inspector.hs`
- [ ] T017 [US1] Populate candidate `tx_id`, `body_hash`, `complete`, and `valid_for_supplied_context` in `nix/wasm/tx-inspector/wasm-tx-inspector/src/Conway/Inspector.hs`
- [ ] T018 [US1] Add evaluated validation check groups and failure rendering in `nix/wasm/tx-inspector/wasm-tx-inspector/src/Conway/Inspector.hs`
- [ ] T019 [US1] Ensure `tx.validate` never returns `result.tx_cbor` or mutating changes in `nix/wasm/tx-inspector/wasm-tx-inspector/src/Conway/Inspector.hs`
- [ ] T020 [US1] Run `just check-validate` and record any fixture limitations in `specs/002-tx-validate/quickstart.md`
...
- [ ] T021 [P] [US2] Add a missing-context smoke request fixture in `specs/001-ledger-functional-layer/fixtures/tx-validate-missing-context-request.json`
- [ ] T022 [US2] Add jq assertions for `status == "incomplete"` and actionable `missing_context` entries in `flake.nix`
- [ ] T023 [US2] Add jq assertions that missing context is reported separately from `failures` in `flake.nix`

### Implementation for User Story 2

- [ ] T024 [US2] Implement missing source-output diagnostics in `nix/wasm/tx-inspector/wasm-tx-inspector/src/Conway/Inspector.hs`
- [ ] T025 [US2] Implement missing network, slot, epoch, and protocol-parameter diagnostics in `nix/wasm/tx-inspector/wasm-tx-inspector/src/Conway/Inspector.hs`
- [ ] T026 [US2] Add path fields for every missing-context item in `nix/wasm/tx-inspector/wasm-tx-inspector/src/Conway/Inspector.hs`
- [ ] T027 [US2] Ensure partially evaluated checks can return both `failures` and `missing_context` in `nix/wasm/tx-inspector/wasm-tx-inspector/src/Conway/Inspector.hs`
- [ ] T028 [US2] Run `just check-validate` and verify the missing-context fixture output in `result-validate-smoke/response.json`
...
- [ ] T029 [P] [US3] Add a producer-context validation smoke request fixture in `specs/001-ledger-functional-layer/fixtures/tx-validate-producer-context-request.json`
- [ ] T030 [US3] Add jq assertions for `resolved_inputs` and `resolved_reference_inputs` in `flake.nix`
- [ ] T031 [P] [US3] Add a producer-id mismatch fixture in `specs/001-ledger-functional-layer/fixtures/tx-validate-producer-mismatch-request.json`
- [ ] T032 [US3] Add jq assertions that mismatched producer evidence produces `status == "rejected"` or invalid-context errors in `flake.nix`

### Implementation for User Story 3

- [ ] T033 [US3] Reuse producer transaction decoding to fill `resolved_inputs` in `nix/wasm/tx-inspector/wasm-tx-inspector/src/Conway/Inspector.hs`
- [ ] T034 [US3] Reuse producer transaction decoding to fill `resolved_reference_inputs` in `nix/wasm/tx-inspector/wasm-tx-inspector/src/Conway/Inspector.hs`
- [ ] T035 [US3] Validate producer map keys against decoded producer transaction ids in `nix/wasm/tx-inspector/wasm-tx-inspector/src/Conway/Inspector.hs`
- [ ] T036 [US3] Report missing output indexes as invalid context in `nix/wasm/tx-inspector/wasm-tx-inspector/src/Conway/Inspector.hs`
- [ ] T037 [US3] Run `just check-input-context` and `just check-validate` to verify input-resolution behavior in `result-input-context-smoke/response.json`
...
- [ ] T040 [P] Add `tx.validate` to the implemented operations table in `gh-docs/api.md`
- [ ] T041 Add `tx-validate-result.schema.json` to the public schema list in `specs/001-ledger-functional-layer/contracts/ledger-functional-api.md`
- [ ] T042 Run Fourmolu formatting check with `just format-check` for `nix/wasm/tx-inspector/wasm-tx-inspector/src/Conway/Inspector.hs`
- [ ] T043 Run PureScript compile with `just ui-check` for `docs/inspector/src/Main.purs`
- [ ] T044 Run full repository verification with `just test` for `flake.nix`
- [ ] T045 [P] If a browser validation panel is added, implement result decoding in `docs/inspector/src/FFI/Json.js`
- [ ] T046 [P] If a browser validation panel is added, implement UI rendering in `docs/inspector/src/Main.purs`
- [ ] T047 If browser validation UI is added, verify it with Playwright in `docs/inspector/tests/tx-identify.spec.mjs`
...
- **US1 MVP**: depends on Phase 2. Delivers the first usable `tx.validate`
  operation.
- **US2 Missing Context**: depends on Phase 2 and can proceed after US1 helpers
  exist. It must remain independently testable by using omitted-context
  fixtures.
...
- **US2 (P2)**: Uses the same result constructors as US1 but must be testable
  with missing-context fixtures alone.
...
- T013 can run in parallel with Haskell planning for US1, but T014 and T015
  both edit `flake.nix` and should be sequenced.
- T021 can run in parallel with Haskell planning for US2, but T022 and T023
  both edit `flake.nix` and should be sequenced.
- T029 and T031 can run in parallel for US3 because they add different fixture
  files; T030 and T032 both edit `flake.nix` and should be sequenced.
- T038, T039, and T040 can run in parallel during polish.
- T045 and T046 can run in parallel if browser UI work is selected.
...
Task: "Implement missing source-output diagnostics in nix/wasm/tx-inspector/wasm-tx-inspector/src/Conway/Inspector.hs"
```
...
2. Add `tx.validate` dispatch and MVP validation result.
3. Add missing-context diagnostics.
4. Add producer transaction input-resolution diagnostics.
5. Add docs and optional browser panel.
